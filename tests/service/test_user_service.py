...
def test_get_all_users_exception(app, db_session, monkeypatch):
...
def test_create_user_db_exception(app, db_session, monkeypatch):
    with app.app_context():
        def raise_exception(_):
            raise Exception("Database error")
...
def test_delete_user_db_exception(app, db_session, monkeypatch):
    with app.app_context():
...
def test_delete_admin_user_raises(app, db_session):
    with app.app_context():
        with pytest.raises(user_service.UnsupportedUserOperationError) as e:
            user_service.delete_user('admin')
        assert str(e.value) == "Cannot delete admin user"

def test_delete_nonexistent_user_raises(app, db_session):
    with app.app_context():
        with pytest.raises(user_service.UnsupportedUserOperationError) as e:
            user_service.delete_user('nonexistent_user')
        assert str(e.value) == "Failed to delete user due to error: User with username nonexistent_user does not exist"

def test_delete_user_with_dependencies_raises(app, db_session):
    with app.app_context():
...
def test_update_user_balance(app, db_session):
    with app.app_context():
...
def test_update_nonexistent_user_balance_raises(app, db_session):
    with app.app_context():
        with pytest.raises(user_service.UnsupportedUserOperationError) as e:
...
def test_get_user_by_username(app, db_session):
    with app.app_context():
...
def test_get_user_by_username_nonexistent(app, db_session):
    with app.app_context():
        user = user_service.get_user_by_username('nonexistent_user')
        assert user is None

def test_get_user_by_username_empty_raises(app, db_session):
    with app.app_context():
        with pytest.raises(user_service.UnsupportedUserOperationError) as e:
            user_service.get_user_by_username('')
        assert str(e.value) == "Username cannot be empty"

def test_get_user_by_username_db_exception(app, db_session, monkeypatch):
    with app.app_context():
